...
Customise your avatar by placing:
  ...
  - journal/*.md     → personal diary entries (RAG knowledge base)
"""
...
FACE_IMG     = str(FACE / "face.jpeg")
WAV2LIP_PTH  = str(WAV2LIP / "checkpoints" / "wav2lip.pth")
PERSONALITY  = str(BEHAVIOR / "behavior.txt")
...
def _load_diary_docs(directory: str) -> list[Document]:
    docs = []
    for f in Path(directory).glob("**/*.md"):
        try:
...
class CopycatApp(ctk.CTk):
    ...
    def _splash_screen(splash: SplashScreen) -> None:
        SplashScreen.set_progress(2.0, "Loading functions ...")
    ...
    def _build_ui(self):
        ...
        self.btn_es = ctk.CTkButton(
            frm_btns, text="Escucha", width=130, height=54,
        ...
        self.btn_en = ctk.CTkButton(
            frm_btns, text="Listen", width=130, height=54,
            font=("Arial", 12, "bold"), state="disabled",
            fg_color=C_IDLE,
        ...
        self.btn_config = ctk.CTkButton(
            frm_btns, text="Configuration", width=130, height=54,
            font=("Arial", 12, "bold"), state="disabled",
            fg_color=C_IDLE,
            command=lambda: self._config()
        )
        self.btn_config.grid(row=0, column=2, padx=10)
    ...
    def _load_models(self):
        ...
        try:
            ...
            embed_ready.wait()
            self._status("Indexing journal/ (RAG)…", C_WARN)
            self._init_rag()
    ...
    def _init_rag(self):
        ...
        if CHROMA_DB.exists() and any(CHROMA_DB.iterdir()):
            ...
            self._log("[RAG] Database not found. Indexing journal/ ...")
            docs = _load_diary_docs(str(JOURNAL))
            if not docs:
                docs = [Document(page_content="Diary empty. Add .md files to journal/")]
